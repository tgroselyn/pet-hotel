...
function* addOwner(action) {
    try {
        yield axios.post('/php-pet/api/post.owner.php', action.payload);
...
function* fetchOwners(action) {
    try {
        let ownerList = yield axios.get('/php-pet/api/get.owner.php');
...
function* removeOwner(action) {
    try {
        yield axios.delete('/php-pet/api/delete.owner.php', action.payload);
        yield put({ type: 'FETCH_OWNERS' });
    } catch (err) {
